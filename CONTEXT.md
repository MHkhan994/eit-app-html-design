...
- **All `<select>` dropdowns are left enabled** (`.calc-select` class, no `disabled` attribute) — every tool screen's dropdown across `accounting/` and `production-cost/` opens and lets you pick a real recorded option. Text/number inputs stay `disabled` since typing a value and getting a computed result is the one thing we don't build. When adding a new dropdown, use `class="calc-input calc-select"` with no `disabled` attribute, matching this standing convention.
